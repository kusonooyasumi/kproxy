      ...
      <div class="dialog-content">
        <button 
          ...
          on:click={async () => {
            if (window.electronAPI) {
              try {
                const newProject = await window.electronAPI.project.initialize();
                if (newProject) {
                  // Initialize project in the store
                  projectState.initialize(newProject);
            ...
            if (window.electronAPI) {
              try {
                ...
                if (result.success && result.project) {
                  // Initialize project in the store
                  projectState.initialize(result.project);
                  // Hide the startup dialog
                  showStartupDialog = false;
                }
              } catch (err) {
        ...
        <button 
          ...
          on:click={() => {
            if (window.electronAPI) {
              ...
              window.electronAPI.startWithoutProject();
              showStartupDialog = false;
            }
          }}
        >
